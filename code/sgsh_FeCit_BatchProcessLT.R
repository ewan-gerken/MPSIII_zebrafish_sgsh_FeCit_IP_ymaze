...
input <- here("data/raw_data")
output <- here("data/processed_data")
...
df[[3]] <- #Convert data nested in "head" into a usable format
  ...
  map(~ spread(.x, X3, X4)) %>%
  map(~ separate(.x,"Subject Identification", paste0("fish",seq(1:8)))) %>%
  map(~ dplyr::select(.x, X1,X2, "Apparatus", paste0("fish",seq(1:8)), "Unit ID")) %>%
...
# Fix that some trials the tracking started before 30s
df[[2]] %<>%
  lapply(function(x) {
    x %>%
      dplyr::filter(X1 >= 30)
...
# DATA WANGLING -----------------------------------------------------------
## STEP 1 - select variables that we are interested in
df <-
  df %>%
  dplyr::select(file_list, Apparatus, `Unit ID`,X1, X4, X5, X6, paste0("fish",seq(1:8))) %>% #select the vars we're intrested in
...
df$time <- seconds(df$time) #fixing time variable
...
df <-
  ...
  mutate(fish_id = ifelse(arena == 1, fish1,
                          ifelse(arena == 2, fish2,
                                 ifelse(arena == 3, fish3,
                                        ifelse(arena == 4, fish4,
                                               ifelse(arena == 5, fish5,
                                                      ifelse(arena == 6, fish6,
                                                             ifelse(arena == 7, fish7,
                                                                    ifelse(arena == 8, fish8,
                                                                           NA))))))))) %>%
...
#ERROR CHECK - if any rows == 999 then something has gone wrong with the previous line of code.
#----- EG: when I ran this I had 8 rows showing up for the filter(zone==999) line.
df_a %>% filter(zone == 999)
df_a %>% filter(time_enter == 999)
df_a %>% filter(time_exit == 999)


#Wrangle data to figure out which way the fish turned
df_a <- df_a %>%
  dplyr::select(file_id, apparatus, unit_id, arena, fish_id, bin, zone, time_enter, time_exit) %>%
