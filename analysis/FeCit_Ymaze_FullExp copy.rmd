...
meta <- read_xlsx("data/ymaze_FeCit_FullExp/2023jan13 ip injection FeCitrate naglu hom x het 6m full exp meta.xlsx") %>% 
...
final_data <- read_csv("data/ymaze_FeCit_FullExp/processed_data/final_output.csv") %>% 
  dplyr::select(-1) %>% 
...
#     bin == 6 ~ "50-60 mins"
#     )) %>% 
#   glmer.nb(total_turns ~ genotype*treatment*bin + (1|start.time),
#          data = .)

#glm_timebyturns %>% saveRDS("data/FeCit_fullexp_glmtimeBytotalturns.rds")

# re-import this object
glm_timebyturns <- read_rds("data/FeCit_fullexp_glmtimeBytotalturns.rds")
...
print(emmeans(glm_timebyturns, ~ genotype * treatment), type ="response") %>% 
  ...
  scale_color_viridis_d(end = 0.8, option = "inferno") +
  scale_fill_viridis_d(end = 0.8, option = "inferno") +
  labs(y = "predicted average number of turns per bin performed by zebrafish",
       x = "genotype", 
...
print(emmeans(glm_reps, ~ genotype), type = "response") %>% 
  ...
  labs(y = "Estimated probability of repetitions\n(LLLL + RRRR)",
       ...
       subtitle = "effect of genotype*treatment p = 0.64"
       )
```
...
print(emmeans(glm_reps, ~ genotype * treatment * bin), type = "response") %>% 
  ...
  labs(y = "Estimated probability of alternation\n(LRLR + RLRL)",
       x = "Time interval", 
       title = "GLM predicted probability of zebrafish performing an repetition\ntetragram due to genotype*treatment*bin", 
       subtitle = "Effect of genotype*treatment*bin p = 0.36"
       )
```
